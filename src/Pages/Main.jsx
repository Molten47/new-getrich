...
const Main = () => {
    ...
    const categories = {
        Appetizers: menus.appetizers,
        "Main Courses": menus.mainCourses,
        Desserts: menus.desserts,
        Beverages: menus.beverages
    };
    ...
    <div className='flex flex-col py-6 px-4 justify-center items-center'>
        <div className='cocktail-menu w-full lg:w-1/2 mt-8 lg:mt-0'>
            ...
            {Object.keys(categories).map((category, index) => (
                <div key={index} className='menu-category mb-8'>
                    <h4 className='text-[#DCCA87] text-3xl md:text-3xl mb-4 font-bold'>{category}</h4>
                    <div className='menu-items flex flex-col gap-6 md:gap-8'>
                        {categories[category].map((menu, idx) => (
                            <div key={idx} className='menu-item flex justify-between'>
                                <div className='menu-item-content'>
                                    <h4 className='text-white text-lg md:text-xl mb-1 md:mb-2'>{menu.title}</h4>
                                    <p className='text-[#aaa] text-sm md:text-base'>{menu.tags}</p>
                                </div>
                                <div className='menu-item-price flex items-center'>
                                    <div className='menu-dash w-6 md:w-9 h-px bg-[#DCCA87] mx-2 md:mx-4'></div>
                                    <p className='text-white text-2xl'>{menu.price}</p>
                                </div>
                            </div>
                        ))}
                    </div>
